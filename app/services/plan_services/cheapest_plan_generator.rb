  ...
  class CheapestPlanGenerator
    ...
    def reconstruct_path(previous)
      ...
      degree_requirements.each do |degree_requirement|
        credits_needed = degree_requirement.credit_hour_amount
      ...
      total_cost = term_assignments.sum { |term| term['cost'] }
    ...
    def calculate_cost(school_node, courses)
      ...
      total_cost = terms.sum { |term| term['cost'] }
      total_cost
    end
    ...
    def save_plan(plan_data)
      Plan.create!(
        ...
        path: plan_data[:path],
        term_assignments: plan_data[:term_assignments]
      )
    end

    def enforce_minimum_university_credits(terms, school_node, term_number)
      assigned_credits = terms.sum { |term| term['credit_hours'] }
      required_credits = school_node.school.minimum_credits_from_school
    ...
    def calculate_term_cost(school_node, courses)
      total_credits = courses.sum(&:credit_hours)
      selected_term = school_node.terms.find { |term| term.credit_hour_minimum <= total_credits && total_credits <= term.credit_hour_maximum }
      if selected_term
        if school_node.school.credit_hour_price.present?
          term_cost = school_node.school.credit_hour_price * total_credits
        else
          term_cost = selected_term.tuition_cost
        end
      else
        if school_node.school.credit_hour_price.present?
          avg_credit_price = school_node.school.credit_hour_price
          term_cost = total_credits * avg_credit_price
        else
